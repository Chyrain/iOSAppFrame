...
//        lastPathComponent], __LINE__, [NSString stringWithFormat:(frmt), ##__VA_ARGS__])
//#define V5LogInfo(frmt, ...)    DDLogInfo(@"<%@:(%d)> %@", [[NSString stringWithUTF8String:__FILE__] \
//        lastPathComponent], __LINE__, [NSString stringWithFormat:(frmt), ##__VA_ARGS__])
//#define V5LogDebug(frmt, ...)   DDLogDebug(@"<%@:(%d)> %@", [[NSString stringWithUTF8String:__FILE__] \
//        lastPathComponent], __LINE__, [NSString stringWithFormat:(frmt), ##__VA_ARGS__])
//#define V5LogVerbose(frmt, ...)   DDLogVerbose(@"<%@:(%d)> %@", [[NSString stringWithUTF8String:__FILE__] \
//        lastPathComponent], __LINE__, [NSString stringWithFormat:(frmt), ##__VA_ARGS__])
...
// 媒体文件缓存路径
#define MEDIA_CACHE_PATH [NSString stringWithFormat:@"%@/mediacache", PATH_OF_DOCUMENT]

// 图片读取
#define ResourcePath(path)  [[NSBundle mainBundle] pathForResource:path ofType:nil]
#define ImageWithPath(path) [UIImage imageWithContentsOfFile:ResourcePath(path)] // 不缓存，省内存，不能获取Assets中的图片
#define ImageWithName(name) [UIImage imageNamed:name]                            // 缓存图片

#define AppLanguage [NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"userLanguage"]]
...
#define kEnglishKeyboardHeight  ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? (216.f) : (312.f))
#define kChineseKeyboardHeight  ((UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone) ? (252.f) : (348.f))
