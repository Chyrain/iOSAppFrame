...
- (void)handlePanAction:(UIPanGestureRecognizer *)pan
{
    if (!_isAnimating)
    {
        if(pan.state == UIGestureRecognizerStateBegan) {
            ...
            NSLog(@"UIGestureRecognizerStateBegan point: %@", NSStringFromCGPoint(point));//////
//            // 这部分代码使r5红点跟着手势走
//            _mHeight = point.y*0.7 + MIN_HEIGHT;
//            self.curveX = self.mStartX = point.x;
...
//                                          _curveView.frame.size.height);
        ...
        else if (pan.state == UIGestureRecognizerStateChanged)
        {
            ...
            CGPoint point = [pan translationInView:self];
            NSLog(@"UIGestureRecognizerStateChanged point: %@", NSStringFromCGPoint(point));//////
            
            // 这部分代码使r5红点跟着手势走
            //_mHeight = point.y + MIN_HEIGHT;//point.y*0.7 + MIN_HEIGHT;
            self.curveX = _startX + point.x; //SYS_DEVICE_WIDTH/2.0 + point.x
            self.curveY = _startY + point.y;//_mHeight > MIN_HEIGHT ? _mHeight : MIN_HEIGHT;
            _curveView.frame = CGRectMake(_curveX,
                                          _curveY,
                                          _curveView.frame.size.width,
                                          _curveView.frame.size.height);
        ...
        {
            NSLog(@"UIGestureRecognizerStateEnded %d", pan.state);//////
            ...
            [UIView animateWithDuration:1.0
                 ...
                 usingSpringWithDamping:0.5
                  initialSpringVelocity:0
                             ...
                             animations:^{
                                 if (self.curveY > self.frame.size.height*0.7) {
                                     _curveView.frame = CGRectMake(SYS_DEVICE_WIDTH/2.0, self.frame.size.height, 3, 3);
                                 } else
...
{
    CGFloat height = MIN_HEIGHT + (_curveY - MIN_HEIGHT) * (_curveY/self.frame.size.height);
    if (_startY >= self.frame.size.height && _curveY >= self.frame.size.height * 0.8) {
        //height = 0 + (self.frame.size.height - _curveY) * (self.frame.size.height - _curveY)/(self.frame.size.height-MIN_HEIGHT);
    }
    ...
    [tPath addLineToPoint:CGPointMake(SYS_DEVICE_WIDTH,  height)];  // r4点
    [tPath addQuadCurveToPoint:CGPointMake(0, height)
